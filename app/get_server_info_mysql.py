...
cpu_use_sql = """select max(id),round(100-idle_use,2),host_ip from sys_info where idle_use is not null group by host_ip"""
mem_use_sql = """select max(id),round((1-(mem_free+mem_buffer)/mem_total)*100,2),host_ip from sys_info where mem_free is not null group by host_ip"""
...
disk_info_sql = """select max(id),lun_name,lun_use,lun_size,lun_rate from sys_info where lun_name is not null and host_ip='{ip}' group by lun_name"""
